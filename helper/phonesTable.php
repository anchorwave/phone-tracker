...
class Phones_Table extends WP_List_Table
{
	...
	function column_name($item)
	{
		if($item['status'] == 1)
		{
			$actions = array(
				...
				'calls'      => sprintf('<a href="?page=tw-calls&p_id=%s">Calls</a>',$item['p_id'])
			);
			if($item['rec_status'] == 1)
			{
				$actions['disrec'] = sprintf('<a href="?page=%s&action=%s&phone=%s">Disable Recording</a>',$_REQUEST['page'],'disrec',$item['p_id']);
			}
			else
			{
		...
		/*$1%s*/ sprintf('<a href="?page=tw-calls&p_id=%s">%s</a>',$item['p_id'],$item['name']),
		/*$3%s*/ $this->row_actions($actions)
		);
	}
	function column_phn_no($item)
	{
		$result = preg_replace('~.*(\d{3})[^\d]*(\d{3})[^\d]*(\d{4}).*~', '+1 ($1) $2-$3', trim($item['phn_no']));
		if(  $result )
		{
			return $result;
		}
	...
	/*function column_dest_no($item)
	{
		if(  preg_match( '/^\+(\d)(\d{3})(\d{3})(\d{4})$/', trim($item['dest_no']),  $matches ) )
		{
	...
	function column_dest_no($item)
	{
		$result = preg_replace('~.*(\d{3})[^\d]*(\d{3})[^\d]*(\d{4}).*~', '+1 ($1) $2-$3', trim($item['dest_no']));
		if(  $result )
		{
			return $result;
		}
	...
	**************************************************************************/
	function process_bulk_action() {
		...
		if(trim($phones) != '')
		{
			...
			{
				$wpdb->query( "DELETE FROM " . PHONES_TABLE . " WHERE p_id in " . $phones );
			}
			elseif('enrec' == $currentAction)
			{
				$wpdb->query( "update " . PHONES_TABLE . " set rec_status = 1 WHERE p_id in " . $phones );
			}
			elseif('disrec' == $currentAction)
			{
				$wpdb->query( "update " . PHONES_TABLE . " set rec_status = 0 WHERE p_id in " . $phones );
			}
			elseif('inactivate' == $currentAction)
			{
				$wpdb->query( "update " . PHONES_TABLE . " set status = 0 WHERE p_id in " . $phones );
			}
			elseif('activate' == $currentAction)
			{
				$wpdb->query( "update " . PHONES_TABLE . " set status = 1 WHERE p_id in " . $phones );
			}
		}
	}
	/** ************************************************************************
	...
	**************************************************************************/
	...
	{
		global $wpdb;
		$query = "SELECT a.* FROM " . PHONES_TABLE . " as a";
	...
	**************************************************************************/
	function get_sortable_columns()
	{
		$sortable_columns = array(
			...
			'ttl_calls'	=> array('ttl_calls',false)
		);
	...
	**************************************************************************/
	function prepare_items()
	{
		...
		$query = "SELECT a.*, (SELECT count(b.c_id) FROM " . CALLS_TABLE . " as b where b.p_id = a.p_id  and b.s_time >= '" . $graph_from . "' and b.s_time < '" . date ("Y-m-d", strtotime("+1 day", strtotime($graph_to))) . "' and b.rec = 1) as ttl_calls FROM " . PHONES_TABLE . " as a";
		if($_GET['post_status'] == 'inactive')
		{
